<?php
function pdvVendasEnsureCustomerSchema(PDO $pdo): void
{
    $pdo->exec(
...
pdvVendasEnsureCustomerSchema($pdo);
...
<div class="max-w-4xl mx-auto">
    <div class="mb-4">
        <h2 class="text-2xl font-bold">PDV Vendas</h2>
    ...
    <form method="post" action="actions/sale_finalize.php" class="bg-white rounded-xl shadow p-4 md:p-6">
        <input type="hidden" name="return_page" value="pdv_vendas">
...
function addItem() {
    ...
    const options = ['<option value="">Selecione o produto</option>'].concat(
        products.map((p) => `<option value="${p.id}" data-price="${Number(p.price)}">${p.name} | Est: ${p.stock_qty} | R$ ${Number(p.price).toFixed(2)}</option>`)
    ).join('');

    wrapper.innerHTML = `
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <select required name="items[product_id][]" class="border rounded px-3 py-3">${options}</select>
    ...
    const select = wrapper.querySelector('select[name="items[product_id][]"]');
    ...
    select.addEventListener('change', () => {
        ...
        if (price > 0 && (!unit.value || Number(unit.value) === 0)) {
